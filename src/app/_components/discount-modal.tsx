...
}: //   setShowModal,
{
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center justify-between px-5 pb-12 pt-10">
        <div className="flex flex-col items-center gap-y-3 text-4xl font-semibold ">
          {t.rich("discount txt", {
            nowrap: (val) => (
              <h2 className="flex flex-row justify-center gap-x-2 whitespace-nowrap">
                {val}
              </h2>
            ),
            important: (val) => <p className="font-bold text-accent">{val}</p>,
          })}
        </div>
        <div className="mt-20 flex flex-col items-center justify-center">
          <Button
            ...
            className="flex h-16 w-72 cursor-pointer items-center justify-center 
            rounded bg-blue-500 px-4 py-3 font-bold text-gray-100 
            shadow hover:bg-blue-600 hover:text-white"
          >
            <svg
              viewBox="0 0 24 24"
              className="mr-6 h-8 w-[13%] fill-current"
            ...
            <span className="w-[90%] pl-3 font-sans text-xl">{t("auth")}</span>
          </Button>
        </div>
      </div>
    </Modal>
